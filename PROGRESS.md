...
**CORS added once the frontend needed it, verified rather than assumed.** The frontend (CloudFront) and the API (API Gateway) are different origins, so every browser `fetch()` call needs `Access-Control-Allow-Origin` or it's silently blocked client-side regardless of what the server returns. Added `CORSMiddleware` to `rest.py` with `allow_origins=["*"]` — wide open on purpose, same reasoning as opening the RDS security group and disabling MCP's DNS-rebinding protection: every route here is an unauthenticated public GET with no cookies or session state, so there's no credential a hostile origin could ride on. Confirmed with a real cross-origin header check against the live deployed API before wiring the frontend against it, not assumed from reading the middleware's docs.

**Frontend: a single static `frontend/index.html`, vanilla JS, no build step, no framework** — matches this project's minimal-tooling approach throughout. Ticker dropdown populated from `/api/tickers` (the curated set, not free text, per this Part's original scope decision), selecting a ticker fetches `/api/reports/{ticker}` and renders each finding's tier, category, reasoning, confidence, and both source excerpts. Field names were cross-checked against a real live API response before writing the render functions (not guessed from the Pydantic models), and the actual templating logic was dry-run in Node against real AAPL data pulled from the live API — this incidentally produced a good sanity check of the two-layer verifier design from Part 5: the first AAPL finding renders as low-tier/0% confidence because the verifier hard-failed it (`excerpt_verified: false`) after the classifier had called it high-tier, and the frontend correctly prioritizes `final_tier` over the raw `tier`, surfacing the verifier's correction rather than the classifier's original (wrong) call.

**S3 + CloudFront set up manually in the console** (matching the RDS precedent — not IaC), bucket kept private with CloudFront Origin Access Control rather than public static website hosting. One real bug found only by actually loading the live URL: the root path (`/`) 403'd while `/index.html` directly returned 200 — `DefaultRootObject` had silently not been saved during the console wizard despite being set in the steps. Fixed directly via `aws cloudfront update-distribution` (faster and more reliable than sending another round of console instructions), then re-verified after waiting for the distribution to finish re-deploying (`Status: Deployed`, not just "update accepted"). `scripts/deploy_frontend.sh` wraps `aws s3 sync` + a CloudFront invalidation for future updates.

**One environment issue hit and fixed along the way, unrelated to the app itself:** the `aws` CLI (an x86_64 binary) stopped running entirely mid-Part with `bad CPU type in executable` — Rosetta 2 was missing from this Apple Silicon Mac. Fixed with `softwareupdate --install-rosetta --agree-to-license` (a standard, reversible Apple-provided compatibility layer, not a risky system change) rather than treating it as a blocker requiring your intervention.

**Real live verification, end to end, from a real browser's perspective (not just curl against the API in isolation):** fetched the live frontend URL directly, confirmed it serves `index.html` with the correct `API_BASE` baked in, then replayed the *exact* cross-origin requests the page's own JS makes (`Origin: https://d30z0su1b3sesp.cloudfront.net` against both `/api/tickers` and `/api/reports/AAPL`) and confirmed both the 200 status and the `Access-Control-Allow-Origin` header the browser actually checks.

- [x] `CORSMiddleware` added to `rest.py` (`allow_origins=["*"]`) + test
- [x] `frontend/index.html` — static page, curated-ticker dropdown, full finding rendering (tier, category, reasoning, confidence, both excerpts)
- [x] `scripts/deploy_frontend.sh` (`aws s3 sync` + CloudFront invalidation)
- [x] S3 bucket + CloudFront distribution with OAC set up (your action, console)
- [x] `DefaultRootObject` bug found and fixed via CLI after a live 403 on `/`
